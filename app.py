...
from flask_wtf.csrf import CSRFProtect # here we used CSRF protection built-in to Flask to avoid CSRF
...
def home():
    if not logged_in():
        return render_template("login.html")# we used templates for HTML to avoid  XSS 
    return redirect('/dashboard')


@app.route("/login", methods=["POST"])# here we insured authentication
def login():
    email = request.form.get("email")
    password = request.form.get("password")
    user = get_user_with_credentials(email, password)
    if not user:
        return render_template("login.html", error="Invalid credentials")# returning error if the email or password is not correct
...
def transfer():# here we ensured authorization 
    if not logged_in():
        return render_template("login.html")
